...
const QurbanProductsDailyTotalChart = () => {
  /* ----------------------------- Start variables ---------------------------- */
    ...
    yaxis: {
      ...
      labels: {
        ...
        formatter: function (val, opt) {
          if (val) {
            return `${(val / 1000).toLocaleString()}K  AUD`
          } else {
            return val
          }
        },
  ...
  /* ------------------------------ Start methods ----------------------------- */
  const getDailyQurbanProductsTotal = async (searchs) => {
    setIsLoading(true);
    let tempSearch = {
    ...
    try {
      const response = await services.dailyQurbanProductsTransactionTotalReport(tempSearch);
      console.log("dailyQurbanProductsTransactionTotalReport", response);
      let temp = [];
      ...
      console.log("compare", temp,tempProductsCount)
      setIsLoading(false);
    } catch (error) {
      setIsLoading(false);
  ...
  const handleReset = () => {
    ...
    setSearchItems(temp);
    getDailyQurbanProductsTotal(temp)
  }
  ...
  /* ------------------------------- Start hooks ------------------------------ */
  useEffect(() => {
    getDailyQurbanProductsTotal(searchItems);
      ...
      <div className="flex flex-row justify-between px-3 pt-2">
        ...
        <div className="mb-6 flex w-full items-center justify-end">
          ...
          <div className="basis-full md:basis-1/4 xl:basis-1/6 pl-1 flex items-end h-full justify-end mt-2 xl:mt-0">
            <button disabled={isLoading} onClick={() => getDailyQurbanProductsTotal(searchItems)} className='flex justify-center items-center text-white text-md font-medium mr-1 rounded-md h-[42px] px-2 hover:bg-[#db346e] bg-[#49acea] transition-all duration-200'>
...
export default QurbanProductsDailyTotalChart;
